...
// variables for custon view point
var viewX = 0;
var viewY = 0;
var viewAngle = 0;
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 56:	// 8: from beside the car
                ...
                break;
            case 57:    // 9. Custom viewpoint
                isHeight = 0;
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "8: Breytanlegt sjónarhorn";
                break;
            case 48:    // 0. from above house looking at car
                isHeight = 1;
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "8: Horfir á bílin frá húsi";
                break;
            case 38:    // up arrow
                changeHeightOrAngle(2);
                document.getElementById("Height").innerHTML = "Viðbótarhæð: "+ height;
                break;
            case 40:    // down arrow
                changeHeightOrAngle(-2);
...
function changeHeightOrAngle(i) {
    if (isHeight == 1) {
        height += i;
    } else {
        viewX  += Math.cos(viewAngle) * 5*i;
        viewY  += Math.sin(viewAngle) * 5*i;
    }
}
...
function render()
{
    ...
    switch( view ) {
    case 0: 
        // Static viewpoint over house, camera flollows car
	...
	case 8:
	    ...
	    break;
    case 9:
        //Custom viewheight and angle
        mv = lookAt( vec3(viewX, viewY, 40), vec3(viewX + 100 * Math.cos(viewAngle),viewY +  100 * Math.sin(viewAngle), 0), vec3(0.0, 0.0, 1.0 ) );
        drawScenery( mv );
        mv = mult( mv, translate(carXPos, carYPos, 0.0) );
        mv = mult( mv, rotateZ( carDirection ) ) ;
        drawCar( mv );
